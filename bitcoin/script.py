...
def add_number_to_script(i: int) -> bytes:
    return bytes.fromhex(push_script(script_num_to_hex(i)))
    ...
    while i < len(_bytes):
        ...
        if opcode <= opcodes.OP_PUSHDATA4:
            ...
            if opcode == opcodes.OP_PUSHDATA1:
                try: nSize = _bytes[i]
                except IndexError: raise Exception("MalformedQtumScript")
                i += 1
            elif opcode == opcodes.OP_PUSHDATA2:
                try: (nSize,) = struct.unpack_from('<H', _bytes, i)
                except struct.error: raise Exception("MalformedQtumScript")
                i += 2
